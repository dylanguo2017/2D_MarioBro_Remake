...
namespace Game
{
    public class SmallSpinFireBeam : IItem
    {
        private Game myGame;
        ...
        public SmallSpinFireBeam(Game game, int x, int y)
        {
            myGame = game;
        ...
        private void loadArray()
        {
            ...
            {
                rotatePosition[i].X =origin.X + stdSpriteSize*(float)Math.Cos(((float)i / countMax) * two * Math.PI);
                rotatePosition[i].Y = origin.Y + stdSpriteSize * (float)Math.Sin(((float)i / countMax) * two * Math.PI);
