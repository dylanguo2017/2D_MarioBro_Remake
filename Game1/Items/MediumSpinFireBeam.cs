...
namespace Game
{
    public class MediumSpinFireBeam : IItem
    {
        private Game myGame;
        ...
        public MediumSpinFireBeam(Game game, int x, int y)
        {
            myGame = game;
        ...
        private void loadArray()
        {
            ...
            {
                rotatePosition[i].X = origin.X + thirtyTwo * (float)Math.Cos(((float)i / countMax) * two * Math.PI);
                rotatePosition[i].Y = origin.Y + thirtyTwo * (float)Math.Sin(((float)i / countMax) * two * Math.PI);
